...
export class KnowledgeBase {
    ...
    searchContracts(query) {
        ...
        return contracts.filter((c) => c.name.toLowerCase().includes(lowerQuery) ||
    ...
    getAllFeatures(filter) {
        ...
        if (filter?.tags && filter.tags.length > 0) {
            features = features.filter(f => filter.tags.some(tag => f.tags.includes(tag)));
        }
        return features;
    }
    /**
    ...
    searchFeatures(query) {
        const data = this.loadFeatures();
        const lowerQuery = query.toLowerCase();
        return data.features.filter(f => f.name.toLowerCase().includes(lowerQuery) ||
    ...
    getFeatureContext(featureId) {
        ...
        const relatedContracts = Object.values(contractsData).filter((c) => feature.relatedContracts.includes(c.id) ||
            feature.relatedContracts.includes(c.name));
        const relatedPatterns = Object.values(patternsData).filter((p) => feature.relatedPatterns.includes(p.id) ||
    ...
    listDocuments(filters) {
        ...
        if (filters?.keywords && filters.keywords.length > 0) {
            docs = docs.filter(d => filters.keywords.some(kw => d.keywords.some(docKw => docKw.toLowerCase().includes(kw.toLowerCase())) ||
    ...
    findSimilarDocuments(title, topics = [], keywords = []) {
        ...
        const results = docs.map(doc => {
            ...
            // Tópicos em comum
            const commonTopics = topics.filter(t => doc.topics.some(dt => dt.toLowerCase().includes(t.toLowerCase()) ||
                t.toLowerCase().includes(dt.toLowerCase())));
            score += commonTopics.length * 20;
            // Keywords em comum
            const commonKeywords = keywords.filter(k => doc.keywords.some(dk => dk.toLowerCase().includes(k.toLowerCase()) ||
     ...
     */
    validateAgainstContracts(code, context) {
        const violations = [];
        // Validar contratos
        ...
        for (const contract of contracts) {
            ...
            if (contract.interfaceCode && !code.includes(contract.name)) {
                ...
                const isCritical = contract.rules.some(r => r.toLowerCase().includes('must') || r.toLowerCase().includes('required'));
