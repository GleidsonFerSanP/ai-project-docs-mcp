# 🤖 Copilot Instructions - Project Docs MCP

> **IMPORTANTE**: Estas instruções devem ser seguidas em TODAS as interações neste projeto.

## 📋 Workflow Obrigatório

### Antes de Qualquer Mudança

1. **Identifique o contexto do arquivo**:
   ```
   @project-docs identify_context {"file_path": "<caminho-do-arquivo>"}
   ```

2. **Obtenha contratos críticos** (se backend/frontend/infra):
   ```
   @project-docs get_contracts {"context": "backend"}
   ```

3. **Consulte contexto completo do projeto**:
   ```
   Use o prompt: @project-docs:project-context
   ```

### Antes de Criar Documentação

1. **SEMPRE verificar se já existe**:
   ```
   @project-docs check_existing_documentation {
     "title": "título proposto",
     "topics": ["tópico1", "tópico2"],
     "keywords": ["keyword1", "keyword2"]
   }
   ```

2. **Se similaridade ≥ 50%**: ATUALIZAR documento existente, NÃO criar novo

3. **Registrar metadata** após criar/atualizar arquivo:
   ```
   @project-docs manage_documentation {
     "action": "create" ou "update",
     "title": "...",
     "file_path": "caminho/relativo.md",
     "topics": [...],
     "keywords": [...],
     "summary": "..."
   }
   ```

### Ao Implementar Código

1. **Validar contra contratos registrados**:
   ```
   @project-docs validate_contract {
     "contract_name": "NomeDoContrato",
     "code": "código a validar"
   }
   ```

2. **Aplicar padrões conhecidos** - consulte patterns com:
   ```
   @project-docs get_feature_context {"feature_id": "feature-relacionada"}
   ```

### Ao Fazer Decisões Arquiteturais

1. **Registrar a decisão**:
   ```
   @project-docs add_decision {
     "title": "Título da decisão",
     "context": "Por que essa decisão foi necessária",
     "decision": "O que foi decidido",
     "alternatives": ["Alt 1", "Alt 2"],
     "positive_consequences": ["Pro 1"],
     "negative_consequences": ["Con 1"]
   }
   ```

## 🎯 Stack do Projeto

- **MCP Server**: TypeScript + @modelcontextprotocol/sdk
- **VS Code Extension**: TypeScript + VS Code API
- **Protocol**: MCP stdio transport
- **Storage**: JSON files (~/.project-docs-mcp/)

## 🏗️ Princípios Arquiteturais

1. **Clean Architecture**: Separação clara entre domínio, aplicação e infra
2. **Single Responsibility**: Cada módulo tem uma responsabilidade única
3. **MCP Standards**: Seguir rigorosamente o protocolo MCP
4. **Zero Config**: Funcionar out-of-the-box sem configuração manual
5. **Cross-platform**: macOS, Linux, Windows

## 🔒 Contratos Críticos

### MCP Server
- Deve usar stdio transport (não HTTP)
- Tools devem retornar formato `{ content: [{ type: 'text', text: string }] }`
- Errors devem ser tratados e retornados como JSON válido

### VS Code Extension
- Deve registrar via `vscode.lm.registerMcpServerDefinitionProvider`
- Deve declarar `mcpServerDefinitionProviders` no package.json
- Deve incluir node_modules do mcp-server no .vsix

## 📝 Convenções de Código

- **TypeScript strict mode** habilitado
- **Imports**: Sempre usar extensão `.js` para imports de arquivos locais
- **Error handling**: Sempre catch e retornar mensagens claras
- **Logging**: Usar console.error para erros, evitar console.log em produção

## 🚫 O Que NUNCA Fazer

- ❌ Criar documentação duplicada sem verificar
- ❌ Modificar contratos sem validar implementações existentes
- ❌ Fazer decisões arquiteturais sem registrá-las
- ❌ Quebrar compatibilidade do protocolo MCP
- ❌ Adicionar dependências sem atualizar package.json do mcp-server

## 📚 Recursos Úteis

- MCP Docs: https://modelcontextprotocol.io
- VS Code API: https://code.visualstudio.com/api
- Este projeto no GitHub: https://github.com/GleidsonFerSanP/ai-project-docs-mcp

---

**Lembre-se**: Este projeto DEVE usar seu próprio MCP para auto-documentação. Pratique o que prega! 🎯
